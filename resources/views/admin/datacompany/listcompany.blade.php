            ...
            <div class="card">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex py-1 align-items-center">
                                            <img src="{{ asset('/storage/company_logo/'. $companyList->company_logo ) }}"
                                    ...
                                    <td>
                                        @if($companyList->trashed())
                                        <form action="{{ route('admin.restorecompany', $companyList->id) }}"
                                            ...
                                            <button class="btn btn-success btn-sm" type="submit">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    ...
                                                    class="icon icon-tabler icon-tabler-rotate-ccw">
                                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                                    <path d="M20 11a8.1 8.1 0 0 0 -15.5 -2m-.5 -5v5h5" />
                                                    <path d="M12 12a8.1 8.1 0 0 0 4.2 7.3" />
                                                    <path d="M12 12v1" />
                                                    <path d="M12 3v9" />
                                                </svg>
                                                Pulihkan
                                        ...
                                        <a class="btn btn-primary btn-sm"
                                            href="{{ route('admin.companyedit', $companyList->id) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                        ...
                                        <a class="btn btn-danger btn-sm" data-bs-toggle="modal"
                                            data-bs-target="#modal-hapus-{{ $companyList->id }}">Hapus</a>
                ...
                <div class="card-footer d-flex align-items-center">
                    <p class="m-0 text-secondary">
                        Menampilkan <span>{{ $company->firstItem() }}</span> hingga
                        <span>{{ $company->lastItem() }}</span> dari <span>{{ $company->total() }}</span> baris
                    </p>
                    <ul class="pagination m-0 ms-auto">
                        <!-- Previous Page Link -->
                        <li class="page-item {{ $company->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $company->previousPageUrl() }}" tabindex="-1"
                                aria-disabled="{{ $company->onFirstPage() }}">
                        ...
                        <li class="page-item {{ $company->hasMorePages() ? '' : 'disabled' }}">
                            <a class="page-link" href="{{ $company->nextPageUrl() }}"
                                aria-disabled="{{ !$company->hasMorePages() }}">
                                Next
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                                    viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                    stroke-linecap="round" stroke-linejoin="round">
